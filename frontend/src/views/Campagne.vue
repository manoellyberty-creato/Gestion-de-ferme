...
// Remplace ton computed actuel par celui-ci :
const campaignsWithRoles = computed(() => {
  // On récupère la donnée brute
  const rawCampaigns = store.campaigns;
  
  // On s'assure d'avoir un tableau, sinon on retourne vide
  const campaignsArray = Array.isArray(rawCampaigns) 
    ? rawCampaigns 
    : (rawCampaigns?.data && Array.isArray(rawCampaigns.data) ? rawCampaigns.data : []);
